...
# MERCHANTABILITYGameToolbar or FITNESS FOR A PARTICULAR PURPOSE.  See the
...
class PlayGo(Activity):
    ...
    def __init__(self, handle):
        ...
        self.collaboration = CollaborationWrapper(
            self, self.buddy_joined, self.buddy_left, self.Play,
            self.game.undostack, self.bootstrap)
    ...
    def set_up_ui(self):
        self.board = boardwidget.GoBoardWidget(
            self.game.get_status(), self.size)
    ...
    def insert_cb(self, widget, x, y, announce=True, ai_play=False):
        ...
        if x != -1:
            ...
            self.board.draw_stone(
                self.board.context, x, y, self.get_currentcolor(), widget)
    ...
    def bootstrap(self, plays):
        ...
        for pos in plays:
            logger.debug(
                'Playing at %s with color %s', pos, self.get_currentcolor())
            captures = self.game.play(
                (pos[0], pos[1]), self.get_currentcolor())
            if captures:
                self.redraw_captures(captures)
